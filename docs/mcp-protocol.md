...
|---|---|
| 传输 | TCP |
| 监听地址 | **默认 `bind("127.0.0.1", 35515)`**（回环）🔴 非回环需显式放开 |
| 暴露方式 | 默认仅经 `adb forward tcp:35515 tcp:35515`；同机/局域网直连时可 `--no-adb` 跳过 |
| PC 侧连接 | `connect(host, 35515)` —— host 由 `--host` 决定，默认 `127.0.0.1` |
| 编码 | UTF-8 |
| 帧格式 | **NDJSON（换行分隔 JSON）**：每条消息一个 JSON 对象，以 `\n` 结尾 |

🔴 **安全理由**：UMT 以 root 运行、提供内存读写接口。**默认只监听回环。**
放开到非回环（`/sdcard/UnrealMemoryTools/mcp_bind.conf` 写入 `192.168.x.x` 或 `0.0.0.0`）
等于把 root 级内存读写暴露给同网段甚至蜂窝网任意设备，仅在可信网络下使用，用完即删配置并重启游戏。
